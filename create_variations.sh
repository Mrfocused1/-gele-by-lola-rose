...
BASE_DIR="/Users/paulbridges/Desktop/Gele by Lola Rose/gele-by-lola-rose/app/api"

# Variation 2: From eyebrows upward
cat > "$BASE_DIR/try-on-2/route.ts" << 'EOF'
...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Edit ONLY the area from the eyebrows upward in the first image. Add the ${geleStyle} gele headwrap from the second image. The gele MUST cover ALL hair completely - absolutely no hair should remain visible. Keep everything from the eyebrows down (eyes, nose, mouth, face, neck, background) exactly unchanged. No strands of hair should be showing.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 2: From eyebrows upward' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 2: From eyebrows upward' });
}
EOF
