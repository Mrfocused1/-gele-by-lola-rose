...
import LoadingScreen from '@/components/try-on/LoadingScreen';
...
export default function TryOnPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      // Get the selected product to find its mannequin image
      const { products } = await import('@/lib/products');
      const selectedProduct = products.find(p => p.id.toString() === selectedStyle);

      if (!selectedProduct) {
        alert('Please select a gele style first.');
      ...
      // Now apply to user's photo using Gemini image generation (experimental)
      const response = await fetch('/api/try-on-gemini', {
        ...
        body: JSON.stringify({
          userImage,
          mannequinImagePath: mannequinImage,
        }),
      });
      ...
      if (response.ok) {
        ...
        // Show a friendly message for AI generation issues
        if (error.error && error.error.includes('No content in Gemini response')) {
          alert('AI generation didn\'t work this time. This can happen occasionally with AI models. Please try again!');
        } else {
          alert('The AI couldn\'t generate the result this time. Please try again with a different photo or gele style.');
        }
      }
    } catch (error) {
  ...
  const handleRetry = () => {
    // Keep same image and style, just retry the generation
    setStep(3); // Stay on results page
    setResultImage(null); // Clear current result
    handleProcess(); // Process again
  };
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimationWrapper animation="fadeUp">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-primary mb-6">
              See Yourself in Gele
            </h1>
            <p className="text-lg text-text-secondary">
              Upload your photo and try on different gele styles virtually using AI technology
            </p>
          </AnimationWrapper>
        </div>
      </section>
      ...
      <section className="py-12 md:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isProcessing && <LoadingScreen message="Creating your gele look..." />}
        </div>
      </section>
