...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Modify ONLY the upper 45% of the first image. Add the ${geleStyle} gele from the second image ensuring complete coverage of all hair - every strand must be hidden. Lower 55% including all facial features must remain unchanged. Zero hair visibility required.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 7: Upper 45%' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 7: Upper 45%' });
}
