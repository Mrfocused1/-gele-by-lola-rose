...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Modify only the top 55% of the first image. Apply the ${geleStyle} gele from the second image to cover every single strand of hair. No hair should be visible in the result. Bottom 45% stays pixel-perfect unchanged. The gele must provide complete hair coverage.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 4: Top 55%' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 4: Top 55%' });
}
