...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Edit ONLY from the hairline upward in the first image. The ${geleStyle} gele from the second image must completely replace and cover ALL hair - zero hair visibility. Keep from the forehead down completely unchanged. The gele should wrap the entire head with no hair exposed.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 3: From hairline upward' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 3: From hairline upward' });
}
