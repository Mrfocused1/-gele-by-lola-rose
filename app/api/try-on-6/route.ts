...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Edit from mid-forehead upward in the first image. The ${geleStyle} gele from the second image must cover the entire hair area completely. Absolutely no hair should be showing. Keep eyes, eyebrows, nose, mouth, and everything below mid-forehead exactly unchanged.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 6: From mid-forehead upward' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 6: From mid-forehead upward' });
}
