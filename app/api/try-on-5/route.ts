...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Edit the hair region only (top portion of head) in the first image. Replace ALL visible hair with the ${geleStyle} gele from the second image. Not a single strand of hair should remain visible. Preserve the face (from mid-forehead down) exactly as is.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 5: Hair region only' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 5: Hair region only' });
}
