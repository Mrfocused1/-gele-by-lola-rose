...
export async function POST(request: Request) {
  try {
    ...
    const geminiRequest = {
      contents: [{
        parts: [
          ...
          { text: `Edit only above the eyebrow line in the first image. Replace the hair with the ${geleStyle} gele from the second image. The gele must cover ALL hair completely - no exceptions, no strands visible. Everything from eyebrows downward stays exactly as original.` }
        ]
      }],
    ...
    return NextResponse.json({ resultImage: resultUpload.secure_url, message: 'Variation 8: Above eyebrow line' });
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ status: 'working', variation: 'Variation 8: Above eyebrow line' });
}
